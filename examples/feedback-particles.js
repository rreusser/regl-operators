...
function run (regl) {
  ...
  const screenbuf = new Array(2).fill(0).map(() => regl.framebuffer({
    color: regl.texture({
  ...
  const initialize = ops.map({
    frag: `
      ...
      varying vec2 uv;
      void main () {
        gl_FragColor = vec4(vec2(0, 28) + (uv - 0.5) + 0.1, 0, 1);
      }
    `
  })
  ...
  const decay = ops.map({
    frag: `
      ...
      void main () {
        ...
        gl_FragColor = color * 0.9 - 0.01;
      }
    `
  })

  const copyWithAlpha = ops.map({
    blend: {
      enable: true,
      func: {
        srcRGB: 'src alpha',
  ...
  regl.frame(() => {
    camera(() => {
      ...
      screenbuf[0].use(() => {
        decay({src: screenbuf[1]})
        splat({src: pos[1]})
      })

      regl.clear({color: [0.2, 0.25, 0.3, 1]})
      copyWithAlpha({src: screenbuf[0]})
